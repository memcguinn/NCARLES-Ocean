! ============================================================================ !
! ABOUT:                                                                       !
!         This subroutine uses the Deardorff TKE model with stability          !
!         corrected length scales to find viscosity. This includes fixes for   !
!         the surface layer and gets the rhs of e-equation.                    !
! ============================================================================ !
!
...
!
    ...
    USE con_stats
!
    REAL :: fnt1(nnx,iys:iye), fnt2(nnx,iys:iye,izs:ize), fnt3(nnx,iys:iye)
    REAL :: ex(nnx,iys:iye), ey(nnx,iys:iye,izs:ize), dissp(nnx,iys:iye)
    REAL :: u_avg(nnx,iys:iye), v_avg(nnx,iys:iye), alk(nnx,iys:iye,izs-1:ize+1)
!
! ---------------------------------------------------------------------------- !
!
! FIND LENGTH SCALES AND EDDY VISCOSITY
  IF (i_dear .EQ. 0) THEN
    CALL dear_vis(alk)
  ELSE
    CALL schu_vis(alk)
  END IF
!
! IF 2-PART SURFACE LAYER MODEL IS ON, FIND MEAN VISCOSITY
  DO iz=izs-1,ize
    izm1         = iz - 1
    izp1         = iz + 1
    vis_mean(iz) = 0.0
  ...
  END DO
!
! UPDATE RHS OF SGS IN SIZE XxZ CUBES (NNX,IYS,IYE,IZS:IZE)
  DO iz=izs,ize
!
    izm1   = iz - 1
    izp1   = iz + 1
...
!
!   INCLUDE STOKES CONTRIBUTION IN ADVECTION AND X-DIFFUSION
    DO iy=iys,iye
      DO ix=1,nnx
...
!
!       VERTICAL DIFFUSION
        fnt3(ix,iy) = ((vis_m(ix,iy,izp1)+vis_m(ix,iy,iz))*                &
...
!       STOKES PRODUCTION
        ...
        r5(ix,iy,iz) = r5(ix,iy,iz)+fnt1(ix,iy)
!
!       BUOYANCY, TAU_W * THETA
        buoy_sgs = -vis_sv(ix,iy,iz)*(t(ix,iy,1,izp1)-t(ix,iy,1,iz))*dzu_i(izp1)
        r5(ix,iy,iz) = r5(ix,iy,iz) + batag*buoy_sgs
      END DO
    END DO
!
!   COMPUTE SHEAR, BUOYANCY, DIFFUSION TERMS IN SGS-E FOR PRINTOUT
    IF (istep .EQ. 1) THEN
      shrz(iz)   = 0.0
      triz(iz)   = 0.0                        ! Vertical diffusion only
      t_diss(iz) = 0.0
      DO iy=iys,iye
        DO ix=1,nnx
...
!
  ...
  END DO
!
! UPDATE TENDENCY OF SGS FROM Y CONTRIBUTIONS, (NNX,IYS:IYW,IZS:IZE)
  DO iz=izs,ize
    DO iy=iys,iye
      DO ix=1,nnx
  ...
  END DO
!
  CALL yd_mpi(ey(1,iys,izs),trigx(1,2),yk(1),nnx,nny,ixs,ixe,ix_s,ix_e,    &
                iys,iye,iy_s,iy_e,izs,ize,myid,ncpu_s,numprocs)
!
! SKEW SYMMETRIC ADVECTION, [vde/dy + d/dy(ve)]/2, AND SFS DIFFUSION
  DO iz=izs,ize
!
    izm1   = iz - 1
    izp1   = iz + 1
